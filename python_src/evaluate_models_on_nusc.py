...
def test_pixel_based_network():
    ...
    ret_mae = sum(maes)/len(maes)
    ret_iou = sum(ious)/len(ious)
    ret_cd = sum(cds)/len(cds)

    return ret_mae, ret_iou, ret_cd


def test_implicit_network(pred_batch=4):
    low_res_index = range(0, 32, 4)
    grid_size = 0.1
    rmax = 80
    maes = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('-d', '--nusc_directory',
                        default='./dataset/nuscenes/val_rv',
                        type=str,
                        required=False,
                        help='Nusc dataset directory')
    ...
    # Test settings
    dataset_config = {'name': 'Nuscenes',
                      'type': 'range_samples_from_image',
                      'args': {'directory': args.nusc_directory,
                               'scene_ids': ['BLABLA'],
                               'res_in': '8_1024',
                               'res_out': '32_1024',
                               'num_of_samples': 8192,
    ...
    print('  ')
    print('  Dataset: Nuscenes', '(' + str(len(test_dataset)) + ' pairs)')
