#!/bin/bash -l 
#SBATCH --export=ALL
#SBATCH --job-name=LIIF_eval               # your job name
#SBATCH --nodes=1                          # 1 node
#SBATCH --ntasks-per-node=1                # one srun task per node
#SBATCH --gres=gpu:a40:1               # 1 GPU on that node
...
apptainer exec --nv \
  ...
  bash -lc '
    ...
    conda activate py38 &&
    cd "$CODE_DIR" &&
    python3 python_src/evaluate_models_on_nusc.py && 
      -cp ./models/trained/liif/liif_lidar_latest.pth
    '